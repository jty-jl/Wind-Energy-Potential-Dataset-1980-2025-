...
1. Hourly Processing: Reads Gross CF, applies 0.85 efficiency, saves Net CF files.
...
# Input Directory
CF_GROSS_DIR = Path("data") / "CF" / "land"

# Output Base Directory
OUT_BASE = Path("data") / "Potential" / "land"

# 1. Output Directory for Hourly Net CF
OUT_CF_HOURLY_DIR = OUT_BASE / "CF_hourly_net_085"
...
# Turbine Parameters
P_RATED_MW = 5.25  # Rated Power (MW)
D_M = 135.0  # Rotor Diameter (m)
EFFICIENCY = 0.85  # Net Efficiency Factor
...
if __name__ == "__main__":
    ...
    for f in files:
        with xr.open_dataset(f) as ds:
            ...
            # Construct dataset manually to control attrs
    ...
    save_da(CF_mon_mean.where(valid2d), DIRS["CF_mon"] / "CF_mon_onshore.nc", "CF_mon")

    # CF Year (Net)
    CF_year_mean = CFh_Net.mean(dim=tname, skipna=True)
    save_da(CF_year_mean.where(valid2d), DIRS["CF_year"] / "CF_year_onshore.nc", "CF_year")
    ...
    # Save combined CD file as per snippet structure
    ds_cd = xr.Dataset(cap_vars)
    ds_cd.to_netcdf(DIRS["CapDen"] / "CapacityDensity_onshore.nc")
    print("Saved: CapacityDensity_onshore.nc")
    ...
    for key, (kx, ky) in SPACINGS.items():
        ...
        for sk, frac in SCENARIOS.items():
            ...
            save_da(Real_mon_total.where(valid2d),
                    DIRS["Real_mon"] / f"Real_mon_{key}_{sk}_onshore.nc",
        ...
        for sk, frac in SCENARIOS.items():
            ...
            save_da(Real_year_total.where(valid2d),
                    DIRS["Real_year"] / f"Real_year_{key}_{sk}_onshore.nc",
